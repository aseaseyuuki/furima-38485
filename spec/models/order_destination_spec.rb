...
RSpec.describe OrderDestination, type: :model do
  describe '購入情報の保存' do
    before do
      ...
      @order_destination = FactoryBot.build(:order_destination, user_id: user.id, item_id: item.id, order_id: order.id)
    end
    ...
    context '内容に問題ない場合' do
      ...
      it "priceとtokenがあれば保存ができること" do
        expect( @order_destinationr).to be_valid
      end
    end
    ...
    context '内容に問題がある場合' do
      ...
      it "tokenが空では登録できないこと" do
        @order_destinationr.token = nil
        @order_destinationr.valid?
        expect( @order_destinationr.errors.full_messages).to include "Token can't be blank"
      end
    end
  end
end
